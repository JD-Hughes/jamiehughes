...
function formatCurrency(num, fixed, fancy) {
    ...
    if (fancy) {
        return '<span class="dsp_cursign">$</span>' + '<span class="dsp_dollars">' + num.split(".")[0] + "</span>" + '<span class="dsp_cents">.' + num.split(".")[1] + "</span>";
    }
    return "$" + num;
}
function formatTime(num) {
    ...
    return String(hour).padStart(2, "0") + ":" + String(mins).padStart(2, "0") + ":" + String(secs.toFixed(0)).padStart(2, "0");
}
...
function addParticipantRow(i) {
    $("#tbl_participants tbody").append(
        '<tr id="participant_' + i.hash + '"><td>' + i.name + '</td><td><button class="btn btn-danger" onclick="removeParticipantRow(\'' + i.hash + "');\">&times; Remove</button></td><tr>"
    );
    setTCPM();
}
